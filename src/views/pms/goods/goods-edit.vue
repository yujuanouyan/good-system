<template>
    <div class="app-container">
        ...
        >
            ...
            <el-row
                v-for="(items, index) in formData.tag"
                :key="'service'+index"
                :gutter="5"
            >
                <el-col :span="4">
                    <el-form-item :label="index ? '' : '服务承诺:'" prop="contents">
                        <el-input v-model="items.contents" />
                    </el-form-item>
                </el-col>
                <el-col :span="2">
                    <el-button v-if="index" @click="removeServiceItem(index)">-</el-button>
                    <el-button v-else @click="addService">+</el-button>
                </el-col>
            </el-row>

            <el-row
                v-for="(item, index) in formData.goodsSpecificationsList"
                :key="'goods'+index"
                :gutter="20"
            >
                <el-col :span="4">
...
export default {
    ...
    methods: {
        ...
        // 添加服务承诺
        addService() {
            this.serviceItemIndex++;
            this.formData.tag.push({
                index: this.serviceItemIndex
            });
        },
            ...
            this.$refs["formData"].validate(valid => {
                if (valid) {
                    ...
                    if (this.queryId) {
                        ...
                        this.$api
                            .editGoods({ content: JSON.stringify(parmas) })
                            .then(res => {
                                if (res.code == 200) {
                        ...
                        this.$api
                            .saveGoods({ content: JSON.stringify(parmas) })
                            .then(res => {
                                if (res.code == 200) {
                                    this.$router.replace("/pms/goods/list");
                                }
        ...
        fetchData(id) {
            this.$api.detailGoods({ goodsExaminId: id }).then(res => {
                if (res.code == 200) {
					...
					this.formData.tag = JSON.parse(res.data.tag)
